        ...
        if(event.getComponentId().equals("startButtonAvaliador")){
            ...
            for(Member roleMember : guild.getMembersWithRoles(role)){
                roleMember.getUser().openPrivateChannel().queue((channel) -> {
                    channel.sendMessage("Um novo ticket para **Avaliador** foi aberto por " + member.getUser().getName() + "\n Verifique!")
                            .queue();
                });
            }


            privateTicket.sendMessage("Olá, " + member.getAsMention() + " Fico contente que tenha optado por fazer uma avaliação na Vulcan Novels para **Avaliador**.\n" +
